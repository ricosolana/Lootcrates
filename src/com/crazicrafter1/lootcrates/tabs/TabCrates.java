...
public class TabCrates extends BaseTabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args){
        ...
        if (args.length > 0) {
            if (args.length == 1) {
                return getMatches(args[0], new String[] {"crate", "detect", "flair", "reload", "version"});
            }
            if (args[0].equals("crate")) {
                if (args.length == 2) {
                    return BaseTabCompleter.getMatches(args[1], Main.crates.keySet().toArray(new String[0]));
                }
                if (args.length == 3) {
